...
export class ProcessChatAnswerUseCase {
  private readonly logger = new Logger(ProcessChatAnswerUseCase.name);

  constructor(
    private readonly prisma: PrismaService,

    private readonly ragServiceClient: RagServiceClient,

    private readonly ragSubscriptionPolicyService: RagSubscriptionPolicyService,
  ) {}

  async execute(ragRequestId: number): Promise<ProcessChatAnswerResult> {
    const request = await this.prisma.ragRequest.findUnique({
      where: {
        ragRequestId,
      },
    ...
    if (!request) {
      throw new NotFoundException(`RagRequest ${ragRequestId} was not found.`);
    }
     ...
     */
    if (request.status !== RagRequestStatus.PROCESSING) {
      this.logger.warn(
    ...
    //   request.turnNumber,
    //   request.conversation.memory?.summarizedUntilTurn ?? 0,
    // );

    const ragResponse = await this.ragServiceClient.generateAnswer({
      question: userMessage.content,

      conversationSummary: request.conversation.memory?.summaryText ?? null,

      isFirstTurn: request.turnNumber === 1,
    });

    const normalizedResponse = this.validateAndNormalizeResponse(
      ragResponse,
      request.turnNumber,
    );
    ...
    const usagePeriod =
      this.ragSubscriptionPolicyService.resolveCurrentMonthlyUsagePeriod(
        request.pharmacySubscription.startsAt,

        request.pharmacySubscription.endsAt,

        request.startedAt,
      );

    const usageDate = this.toUtcDateOnly(request.startedAt);

    const latencyMs = Math.max(
      0,
      finishedAt.getTime() - request.startedAt.getTime(),
    );
    ...
    return this.prisma.$transaction(
      async (tx): Promise<ProcessChatAnswerResult> => {
        ...
        const lockedRequest = await tx.ragRequest.findUnique({
          where: {
            ragRequestId,
          },
         ...
         * استدعاء خدمة RAG.
         */
        if (lockedRequest.status !== RagRequestStatus.PROCESSING) {
          return {
            ragRequestId,
            outcome: 'SKIPPED',
            assistantMessageId: null,
        ...
        const assistantMessage = await tx.ragMessage.create({
          data: {
            ragRequestId,
            ...
            citations:
              normalizedResponse.citations.length > 0
                ? {
                    create: normalizedResponse.citations.map((citation) => ({
        ...
        await tx.ragConversationMemory.upsert({
          where: {
            ragConversationId: lockedRequest.ragConversationId,
          },

          create: {
            ragConversationId: lockedRequest.ragConversationId,

            summaryText: normalizedResponse.updatedSummary,

            summarizedUntilTurn: lockedRequest.turnNumber,
        ...
        if (
          ...
          normalizedResponse.conversationTitle
        ) {
          await tx.ragConversation.updateMany({
            ...
            data: {
              title: normalizedResponse.conversationTitle,
            },
          });
        }
        ...
        await tx.ragUsageDaily.upsert({
          where: {
            pharmacySubscriptionId_usageDate_usagePeriodStart: {
              pharmacySubscriptionId: request.pharmacySubscriptionId,
        ...
        return {
          ...
          assistantMessageId: assistantMessage.ragMessageId,
        };
      },
      {
        timeout: 10_000,
  ...
  async markFailedAfterFinalAttempt(
    ...
    await this.prisma.$transaction(
      async (tx) => {
        ...
        const request = await tx.ragRequest.findUnique({
          where: {
            ragRequestId,
          },
        ...
        const normalizedFailureCode =
          failureCode.trim().slice(0, 100) || 'RAG_PROCESSING_FAILED';
        ...
        if (request.turnNumber === 1) {
          ...
          if (userMessage) {
            await tx.ragConversation.updateMany({
              ...
              data: {
                title: this.createFallbackTitle(userMessage.content),
              },
            });
          }
        }
  ...
  // ): Promise<GenerateRagAnswerInput['recentMessages']> {
  //   if (currentTurnNumber <= 1) {
  //     return [];
  //   }

  //   const previousTurns = await this.prisma.ragRequest.findMany({
  //     where: {
  //       ragConversationId,
  //       status: RagRequestStatus.SUCCEEDED,

  //       turnNumber: {
  //         gt: summarizedUntilTurn,
  //         lt: currentTurnNumber,
  //       },
  //     },
  ...
  //         where: {
  //           role: {
  //             in: [RagMessageRole.USER, RagMessageRole.ASSISTANT],
  //           },
  //         },
  ...
  //   return previousTurns.reverse().flatMap((turn) =>
  ...
  private validateAndNormalizeResponse(
    ...
    const updatedSummary = response.updatedSummary?.trim();
    ...
    const conversationTitle =
      turnNumber === 1 ? this.normalizeTitle(response.conversationTitle) : null;
    ...
    const citations = Array.isArray(response.citations)
      ? response.citations.map((citation, index) => ({
          ...
          position:
            Number.isInteger(citation.position) && citation.position > 0
              ? citation.position
              : index + 1,

          sourceType: citation.sourceType?.trim().slice(0, 50) || 'UNKNOWN',

          documentId: citation.documentId?.trim().slice(0, 255) ?? null,

          chunkId: citation.chunkId?.trim().slice(0, 255) ?? null,

          title: citation.title?.trim().slice(0, 255) ?? null,

          snippet: citation.snippet?.trim() ?? null,
        }))
      : [];
  ...
  private normalizeTitle(title: string | null): string | null {
    ...
    const normalized = title.replace(/\s+/g, ' ').trim().slice(0, 150);
