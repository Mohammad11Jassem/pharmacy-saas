...
export class CreateReturnInvoiceUseCase {
  ...
  async execute(pharmacyId: number, dto: CreateReturnInvoiceDto) {
    ...
    return this.unitOfWork.executeSerializable(async (tx) => {
      ...
      await this.reconcileSaleInvoicePaymentAfterReturn(
        tx,
        referenceSaleInvoice.saleInvoiceId,
      );
  ...
  private async reconcileSaleInvoicePaymentAfterReturn(
    tx: Prisma.TransactionClient,
    saleInvoiceId: number,
  ): Promise<void> {
    const saleInvoice = await tx.saleInvoice.findUnique({
      where: {
        saleInvoiceId,
      },
      select: {
    ...
    const totalAmountCents = this.toCents(Number(saleInvoice.totalAmount));

    const returnedAmountCents = saleInvoice.returns.reduce(
      (sum, returnInvoice) =>
        sum + this.toCents(Number(returnInvoice.subtotalRefund)),
      0,
    );

    /*
     * صافي قيمة الفاتورة بعد جميع المرتجعات المنشورة.
     */
    const netTotalAmountCents = Math.max(
      totalAmountCents - returnedAmountCents,
      0,
    );

    const currentPaidAmountCents = this.toCents(Number(saleInvoice.paidAmount));

    /*
     * لا يمكن أن يبقى المبلغ المدفوع أكبر من صافي الفاتورة.
     * الفرق هنا يمثل المبلغ الذي أُعيد للعميل.
     */
    const adjustedPaidAmountCents = Math.min(
    ...
    if (netTotalAmountCents === 0) {
      paymentStatus = PaymentStatus.PAID;
    } else if (adjustedPaidAmountCents === 0) {
      paymentStatus = PaymentStatus.PENDING;
    } else if (adjustedPaidAmountCents < netTotalAmountCents) {
    ...
    await tx.saleInvoice.update({
      ...
      data: {
        paidAmount: this.fromCents(adjustedPaidAmountCents),
        paymentStatus,
      },
    });
  }
